...
! function () {
  ...
  function g(t) {
    u.put(t), m.searchInput.addEventListener("keyup", function (t) {
      -1 === [13, 16, 20, 37, 38, 39, 40, 91].indexOf(t.which) && (O(), S(t.target.value))
    })
  }
  ...
  function S(t) {
    var e;
    (e = t) && 0 < e.length && (O(), function (t, e) {
      var n = t.length;
      if (0 === n) return z(m.noResultsText);
      for (var r = 0; r < n; r++) t[r].query = e, z(i.compile(t[r]))
    }(u.search(t), t))
  }
  ...
  t = window, m = {
    ...
    var n;
    return 0 < w.validate(t).length && j("You must specify the following required options: " + v), m = h.merge(m, t), i.setOptions({
    ...
    }), h.isJSON(m.json) ? g(m.json) : (n = m.json, d.load(n, function (t, e) {
      t && j("failed to get JSON (" + n + ")"), g(e)
    })), {
      search: S
    }
